...
const WhyChooseUs = () => {
  ...
  return (
    <section className="py-12 md:py-20 px-4 md:px-10 lg:px-24 bg-[#F4F9F9]">
      <Container>
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          
          {/* LEFT COLUMN — TWO LARGE CARDS (Order changed for mobile flow) */}
          ...
          <div className="lg:col-span-8 flex flex-col gap-8 lg:gap-10 order-1 lg:order-2">
            {/* HEADING + PARAGRAPH */}
            <div className="flex flex-col lg:ml-10 gap-4 text-center lg:text-left">
              <h2 className="text-3xl md:text-5xl font-heading font-medium text-gray-800">
...
const LargeCard = ({ title, description, image, icon, floating }) => (
  <div className="relative h-[300px] md:h-[350px] lg:h-[325px] rounded-[28px] overflow-hidden shadow-lg group">
    <img src={image} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt={title} />
    <div className="absolute inset-0 bg-gradient-to-r from-white/95 via-white/80 to-transparent"></div>

    <div
      className={`absolute p-6 w-[90%] sm:max-w-[85%] transition-all duration-300
...
const SmallCard = ({ title, description, image, icon }) => (
  <div className="relative min-h-[220px] sm:h-[200px] rounded-[24px] overflow-hidden border border-[#E3E6E6] group">
    <img src={image} className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt={title} />
    <div className="absolute inset-0 bg-gradient-to-r from-white/95 via-white/80 to-transparent"></div>
